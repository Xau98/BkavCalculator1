...
public class CalculatorExpressionEvaluator {
    ...
    public void evaluate(String expr, EvaluateCallback callback) {
        ...
        try {
            ...
            if (Double.isNaN(result)) {
                ...
                // The arity library uses floating point arithmetic when
                // evaluating the expression
                // leading to precision errors in the result. The method
                // doubleToString hides these
                // errors; rounding the result by dropping N digits of
                // precision.
    ...
    private String addCommas(String digits) {
        ...
        if (digits.length() <= 3)
            return digits; // If the original value has 3 digits or less it
                           // returns that value

        for (int i = 0; i < (digits.length() - 1) / 3; i++) {

            int commaPos = digits.length() - 3 - (3 * i); // comma position in
                                                          // each cicle
